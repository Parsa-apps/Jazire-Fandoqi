...
2. پین قدیمی `image_cropper_platform_interface` با حذف cropper نیتیو و استفاده از Photo Picker سیستم برطرف شد؛ فاصله Flutter 3.24.3 با stable همچنان بدهی SDK است.

**پیشنهاد ترتیب:** اول آپگرید Flutter (یک PR جدا، با تست کامل) → سپس پکیج‌ها یکی‌یکی؛ `hive→hive_ce` قبل از بقیه چون ریسک داده دارد.
