...
## ✅ اجرای ادامهٔ حرفه‌ای — فاز ۳۱، ۳۳، ۳۴ و ۳۵

در ادامهٔ همین audit، سه فیکس حیاتی و سپس فاز ۳۵ اجرا شدند:

- فاز ۳۱: تایمر `MemoryMatchGame` با `Timer.periodic` در `inactive/paused` کاملاً متوقف و در `resumed` ادامه‌پذیر شد.
- فاز ۳۳: تعداد قلب‌های Star Catch و سقف پاورآپ از ثابت مشترک `maxLives = 5` خوانده می‌شود.
- فاز ۳۴: `DrawingGame` مسیر هر stroke را هنگام کشیدن به‌صورت incremental می‌سازد، نقاط تکراری را حذف می‌کند و canvas را در `RepaintBoundary` قرار می‌دهد.
- فاز ۳۵: `PuzzleGame` با چهار اندازهٔ ۴/۶/۹/۱۲ تکه، سه تم آفلاین، DragTarget نوع‌دار، شناسهٔ مستقل تکه‌ها، progress و پاداش امن تکمیل شد.

جزئیات اجرایی در `docs/PHASE_31_35_HOTFIX.md` ثبت شده است.
