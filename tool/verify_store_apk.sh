...
# ── ۲) درگاه پرداخت مایکت باید حاضر باشد ─────────────────────────────
if echo "$MANIFEST" | grep -qi "ir.mservices.market.BILLING"; then
  echo "✅ ۲) درگاه پرداخت مایکت حاضر است (فایل واقعاً بیلد مایکت است)"
else
  echo "⚠️  ۲) هشدار: permission پرداخت مایکت در این فایل پیدا نشد."
