...
/// اصلی است و گرید تیرهٔ قبلی حذف شده تا کودک فقط یک تصمیم روشن
/// داشته باشد: لمس سکوی لالایی‌ای که دوست دارد.
/// ═══════════════════════════════════════════════════════════════
...
class _LullabyHubScreenState extends State<LullabyHubScreen>
  ...
  void initState() {
    ...
    // صفحه برای کودک خردسال خلوت و متمرکز باشد (مثل قصه‌خانه).
    FandoghiCoach.disablePersistentPresence();

    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      FandoghiCoach.say(
        'به لالایی‌خانه خوش اومدی 🌙 یکی از سکوهای لالایی رو لمس کن تا با صدای آروم برات بخونم...',
...
class _LullabyTopBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Semantics(
      container: true,
      label: 'لالایی‌خانه فندقی، $totalCount لالایی',
      child: Container(
        height: 64,
...
class _LullabyPlatformState extends State<_LullabyPlatform>
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      button: true,
      label:
          '${widget.lullaby.title}${widget.locked ? '، قفل' : ''}',
