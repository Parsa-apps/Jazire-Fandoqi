...
/// ═══════════════════════════════════════════════════════════════
/// 🌙 LULLABY HUB SCREEN — لالایی‌کدهٔ جزیره‌ای
///
...
/// اصلی است (با چرخهٔ شب، چون وقتِ لالایی شب است) و خبری از گرید
/// تیره، بنر و کارت‌های تودرتو نیست. کودک خردسال فقط یک تصمیم دارد:
/// لمس سکوی لالایی‌ای که دوست دارد.
/// ═══════════════════════════════════════════════════════════════
...
class _LullabyHubScreenState extends State<LullabyHubScreen>
  ...
  void initState() {
    ...
    // برای کودک خواب‌آلود آرام و خلوت بماند (مثل قصه‌خانه و کارتون‌کده).
  ...
  Widget build(BuildContext context) {
    const lullabies = LullabiesData.all;
    final listenedCount = lullabies
        .where((lullaby) => GameData.hasListenedLullaby(lullaby.id))
        .length;
        ...
        children: [
          Positioned.fill(
            child: IslandMapBackground(
              ...
              // وقتِ لالایی همیشه شب است: همان نقشهٔ جزیره، با آسمانِ شب و ماه.
              ...
              builder: (context, constraints) {
                ...
                return SingleChildScrollView(
                  ...
                  child: _LullabyIslandMap(
                    ...
                    onToggleFavorite: (lullaby) {
                      HapticFeedback.selectionClick();
                      GameData.toggleLullabyFavorite(lullaby.id);
                      setState(() {});
                    },
                  ),
                );
...
class _LullabyTopBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Semantics(
      container: true,
      label:
          'لالایی‌کده فندقی، $listenedCount لالایی از $totalCount لالایی گوش داده شده',
      child: Container(
        height: 64,
...
class _LullabyIslandMap extends StatelessWidget {
  ...
  final ValueChanged<Lullaby> onToggleFavorite;
          ...
          for (var index = 0; index < lullabies.length; index++)
            Positioned(
              ...
              child: _LullabyPlatform(
                ...
                isFavorite: GameData.isLullabyFavorite(lullabies[index].id),
                onTap: () => onLullabyTap(lullabies[index]),
                onToggleFavorite: () => onToggleFavorite(lullabies[index]),
              ),
            ),
        ],
      ),
...
class _LullabyPlatformState extends State<_LullabyPlatform>
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      button: true,
      label:
          '${widget.lullaby.title}${widget.locked ? '، قفل' : ''}${widget.listened ? '، گوش داده شده' : ''}',
                ...
                Positioned(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(
                          color: widget.lullaby.themeColor.withOpacity(0.35),
                          blurRadius: 13,
  ...
  Widget _favoriteButton() {
    return Semantics(
      ...
      label: widget.isFavorite
          ? '${widget.lullaby.title}، حذف از علاقه‌مندی‌ها'
          : '${widget.lullaby.title}، افزودن به علاقه‌مندی‌ها',
      child: GestureDetector(
