...
class FandoghiPosition extends ValueNotifier<Offset> {
  ...
  void updateFromPixels(Offset pixel, Size size) {
    final cx = (pixel.dx / size.width).clamp(0.05, 0.95);
    final cy = (pixel.dy / size.height).clamp(0.08, 0.95);
    value = Offset(cx, cy);
  }
}
...
class _BunnyContainer extends StatefulWidget {
  final double size;
  final Widget child;
  const _BunnyContainer({required this.size, required this.child});

  @override
  State<_BunnyContainer> createState() => _BunnyContainerState();
}

class _BunnyContainerState extends State<_BunnyContainer> {
  static bool _everDragged = false;
  bool _showHint = true;

  @override
  void initState() {
    super.initState();
    if (_everDragged) _showHint = false;
  }
